...
Unified workflow for testing harxitflow installation across multiple platforms, supporting both manual and programmatic execution.

## Manual Testing

### 1. Test from PyPI
Tests published harxitflow packages from PyPI across all platforms.
...
gh workflow run cross-platform-test.yml \
  ...
  -f harxitflow-version="1.0.18"
```

### 2. Test from Source
Builds and tests harxitflow from current branch source code using release-like dependency resolution (transforms workspace dependencies to published packages for testing parity).
...
1. **Package Installation**: `uv pip install harxitflow` (PyPI) or local wheel installation
2. **Dependencies**: Additional packages like `openai` for full functionality
3. **CLI Help**: `harxitflow --help`
4. **Server Startup**: `harxitflow run --backend-only` with `/health_check` endpoint validation
5. **Python Import**: `import harxitflow`
...
|------------|----------------|--------------|----------|---------|
| **Manual/Source** | Workspace (`harxitflow-base = { workspace = true }`) | 162 packages | ❌ Not included | ✅ Success |
| **Nightly/Release** | Published (`harxitflow-base-nightly==0.5.0.dev21`) | 420 packages | ✅ Included | ❌ Compilation fails |
...
2. **Nightly builds** modify dependencies via `scripts/ci/update_uv_dependency.py`:
   - Changes: `harxitflow-base~=0.5.0` → `harxitflow-base-nightly==0.5.0.dev21`
