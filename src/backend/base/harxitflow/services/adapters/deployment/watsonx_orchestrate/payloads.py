...
class WatsonxFlowArtifactProviderData(BaseModel):
    ...
    project_id: NormalizedId = Field(description="HarxitFlow project id carried for watsonx snapshot creation.")
...
class WatsonxRenameToolOperation(BaseModel):
    """Rename a HarxitFlow-managed tool on the provider."""
...
class WatsonxAgentExecutionResultData(BaseModel):
    ...
    agent_id: NormalizedId | None = Field(
        default=None,
        description="WXO agent identifier (resource_key in HarxitFlow DB).",
    )
