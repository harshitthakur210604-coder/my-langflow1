...
from harxitflow.services.adapters.deployment.watsonx_orchestrate.core.shared import (
...
async def _apply_tool_renames(
    *,
    ...
    2. Tool must be a HarxitFlow-managed tool (has ``binding.harxitflow``).
    ...
    if agent_tool_ids:
        ...
        for tool in existing_tools or []:
            if not isinstance(tool, dict):
                continue
            for app_id, connection_id in extract_harxitflow_connections_binding(tool).items():
