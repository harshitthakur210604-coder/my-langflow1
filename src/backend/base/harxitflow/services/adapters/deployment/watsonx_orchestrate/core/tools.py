...
from ibm_watsonx_orchestrate_core.types.tools.harxitflow_tool import create_harxitflow_tool
...
from harxitflow.services.adapters.deployment.watsonx_orchestrate.utils import (
    dedupe_list,
    normalize_wxo_name,
    raise_as_deployment_error,
    require_tool_id,
)
from harxitflow.utils.version import get_version_info

if TYPE_CHECKING:
    from ibm_watsonx_orchestrate_core.types.tools.harxitflow_tool import HarxitFlowTool
    from lfx.services.adapters.deployment.schema import BaseFlowArtifact, SnapshotItems, SnapshotListResult

    from harxitflow.services.adapters.deployment.watsonx_orchestrate.types import WxOClient
...
def ensure_harxitflow_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Ensure ``binding.harxitflow.connections`` exists in *tool_payload* and return the mutable dict.
    ...
    harxitflow = _ensure_dict(binding, "harxitflow")
    return _ensure_dict(harxitflow, "connections")


def verify_harxitflow_owned(tool: dict[str, Any], *, tool_id: str) -> None:
    """Raise ``InvalidContentError`` if the tool lacks ``binding.harxitflow``.

    Call before any mutating operation on an existing tool to ensure
    HarxitFlow created it.  Tools created manually in the wxO console or
    by other integrations will not have this marker.
    """
    binding = tool.get("binding")
    if not isinstance(binding, dict) or "harxitflow" not in binding:
        msg = f"Cannot modify tool '{tool_id}': it does not have a HarxitFlow binding and may not be managed by HarxitFlow."
        raise InvalidContentError(message=msg)


def extract_harxitflow_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Extract ``binding.harxitflow.connections`` from a provider tool payload.
    ...
    if not isinstance(binding, dict):
        return {}
    harxitflow = binding.get("harxitflow")
    if not isinstance(harxitflow, dict):
        return {}
    connections = harxitflow.get("connections")
...
def extract_harxitflow_artifact_from_zip(artifact_zip_bytes: bytes, *, snapshot_id: str) -> dict[str, Any]:
    """Read and parse the HarxitFlow flow JSON from a wxO snapshot artifact zip."""
    try:
...
def build_harxitflow_artifact_bytes(
    *,
    tool: HarxitFlowTool,
    ...
    logger.debug("build_harxitflow_artifact_bytes: filename='%s', requirements=%s", filename, requirements)
    ...
    *,
    ...
    if not flow_definition.get("last_tested_version"):
        ...
        if not detected_version:
            msg = "Unable to determine running HarxitFlow version for snapshot creation."
            raise InvalidContentError(message=msg)
        flow_definition["last_tested_version"] = detected_version

    tool: HarxitFlowTool = create_harxitflow_tool(
    ...
    (tool_payload.setdefault("binding", {}).setdefault("harxitflow", {})["project_id"]) = project_id
    logger.debug(
        "create_wxo_flow_tool: tool name='%s', project_id='%s', binding=%s",
        tool_payload.get("name"),
        project_id,
        tool_payload.get("binding", {}).get("harxitflow"),
    )

    artifacts: bytes = build_harxitflow_artifact_bytes(
        tool=tool,
...
async def process_raw_flows_with_app_id(
    ...
    """Create harxitflow tools in wxO and connect them to the given app_id."""
...
async def verify_tools_by_ids(
    ...
    for tool in tools or []:
        if not isinstance(tool, dict) or not tool.get("id"):
            continue
        connections = extract_harxitflow_connections_binding(tool)
        ...
        if len(normalized_connections) < len(connections):
            logger.warning(
                "Tool %s returned malformed harxitflow connection bindings; defaulting to empty mapping",
                tool["id"],
            )
