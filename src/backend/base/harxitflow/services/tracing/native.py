...
execution traces directly in HarxitFlow's database, enabling the Trace View
...
from harxitflow.serialization.serialization import serialize
...
from harxitflow.services.tracing.span_sorting import (
    HARXITFLOW_SPAN_NAMESPACE,
    resolve_span_uuids,
...
class NativeTracer(BaseTracer):
    """Tracer that stores execution traces in HarxitFlow's database.

    This tracer captures:
    - Component-level traces (via add_trace/end_trace)
    - LangChain-level traces (via get_langchain_callback)

    Enabled by default. Disable with HARXITFLOW_NATIVE_TRACING=false if needed.
    """
    ...
    def _is_enabled() -> bool:
        ...
        return os.getenv("HARXITFLOW_NATIVE_TRACING", "true").lower() not in ("false", "0", "no")
    ...
    async def _flush_to_database(self, error: Exception | None = None) -> None:
        ...
        try:
            ...
            from harxitflow.services.database.models.traces.model import SpanTable, TraceTable

            try:
                flow_uuid = UUID(self.flow_id)
            except (ValueError, TypeError):
                # Deterministic fallback so malformed flow_ids don't silently discard trace data.
                flow_uuid = uuid5(HARXITFLOW_SPAN_NAMESPACE, f"invalid-flow-id:{self.flow_id}")
                logger.error(
            ...
            from harxitflow.services.tracing.formatting import safe_int_tokens
    ...
    def get_langchain_callback(self) -> BaseCallbackHandler | None:
        ...
        from harxitflow.services.tracing.service import component_context_var
        ...
        if component_context:
            ...
            parent_span_id = uuid5(HARXITFLOW_SPAN_NAMESPACE, f"{self.trace_id}-{component_id}")
        elif self._current_component_id:
            # Fallback for edge cases where component context might not be set
            parent_span_id = uuid5(HARXITFLOW_SPAN_NAMESPACE, f"{self.trace_id}-{self._current_component_id}")
    ...
    def end_langchain_span(
        ...
        # Default to chat since most LLM usage in HarxitFlow is conversational
    ...
    def _map_trace_type(trace_type: str) -> SpanType:
        """Normalise HarxitFlow's string trace types to the SpanType enum, defaulting to CHAIN for unknown values."""
