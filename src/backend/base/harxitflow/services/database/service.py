...
from harxitflow.services.database import models
from harxitflow.services.database.constants import (
...
from harxitflow.services.database.session import NoopSession
from harxitflow.services.database.utils import Result, TableResults
from harxitflow.services.deps import get_settings_service
from harxitflow.services.utils import teardown_superuser
...
class DatabaseService(Service):
    ...
    def __init__(self, settings_service: SettingsService):
        ...
        # This file is in harxitflow.services.database.manager.py
        # the ini is in harxitflow
        harxitflow_dir = Path(__file__).parent.parent.parent
        self.script_location = harxitflow_dir / "alembic"
        self.alembic_cfg_path = harxitflow_dir / "alembic.ini"
        ...
        if self.alembic_log_to_stdout:
            ...
            self.alembic_log_path = Path(harxitflow_dir) / alembic_log_file
    ...
    async def ensure_postgresql_version(self) -> None:
        ...
        HarxitFlow's schema uses UNIQUE NULLS DISTINCT, which is only supported in PostgreSQL 15+.
