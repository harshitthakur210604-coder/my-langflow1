...
from harxitflow.cli.progress import create_harxitflow_progress
from harxitflow.initial_setup.setup import get_or_create_default_folder
from harxitflow.main import setup_app
from harxitflow.services.auth.utils import get_current_user_from_access_token
from harxitflow.services.database.models.api_key.crud import check_key
from harxitflow.services.database.service import UnsupportedPostgreSQLVersionError, check_postgresql_version_sync
from harxitflow.services.deps import get_db_service, get_settings_service, is_settings_service_initialized, session_scope
from harxitflow.services.utils import initialize_services
from harxitflow.utils.version import fetch_latest_version, get_version_info
from harxitflow.utils.version import is_pre_release as harxitflow_is_pre_release
...
try:
    ...
    lfx_app = typer.Typer(name="lfx", help="HarxitFlow Executor commands")
...
def run(
    *,
    ...
    log_level = (log_level or os.environ.get("HARXITFLOW_LOG_LEVEL") or "info").lower()
    os.environ["HARXITFLOW_LOG_LEVEL"] = log_level
    ...
    progress = create_harxitflow_progress(verbose=verbose)

    # Step 0: Initializing HarxitFlow
    ...
    with progress.step(1):
        for key, value in os.environ.items():
            new_key = key.replace("HARXITFLOW_", "")
    ...
    if platform.system() == "Windows":
        ...
        from harxitflow.helpers.windows_postgres_helper import HARXITFLOW_DATABASE_URL, POSTGRESQL_PREFIXES

        db_url = os.environ.get(HARXITFLOW_DATABASE_URL, "")
        loop_type = "asyncio"
        if (
        ...
        with progress.step(6):
            # Use Gunicorn with HarxitFlowUvicornWorker for non-Windows systems
            ...
            options = {
                ...
                "preload_app": os.environ.get("HARXITFLOW_GUNICORN_PRELOAD", "false").lower() == "true",
            }
            server = HarxitFlowApplication(app, options)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "harxitflow")
        'A new version of harxitflow is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=harxitflow_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if harxitflow_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use harxitflow.utils.version to get the version info
    version_info = get_version_info()
    harxitflow_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= harxitflow_is_pre_release(harxitflow_version)  # Update pre-release status

    notice = build_version_notice(harxitflow_version, package_name)
    ...
    info_text = (
        f"{github_icon} GitHub: Star for updates {arrow} https://github.com/harxitflow-ai/harxitflow\n"
    ...
    telemetry_text = (
        ...
        if os.getenv("DO_NOT_TRACK", os.getenv("HARXITFLOW_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve HarxitFlow.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_host = get_best_access_host(host, port)
    access_link = f"[bold]{status_icon} Open HarxitFlow {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
    ...
    try:
        ...
        fallback_message = (
            ...
            "* GitHub: https://github.com/harxitflow-ai/harxitflow\n"
            "# Discord: https://discord.com/invite/EqksyE2EX9\n\n"
            f"{telemetry_text}\n\n"
            f"[OK] Open HarxitFlow -> {protocol}://{access_host}:{port}"
        )
        try:
            ...
            logger.info("GitHub: https://github.com/harxitflow-ai/harxitflow")
            logger.info("Discord: https://discord.com/invite/EqksyE2EX9")
            logger.info(f"Open HarxitFlow: {protocol}://{access_host}:{port}")


@app.command()
def superuser(
    username: str = typer.Option(
        None, help="Username for the superuser. Defaults to 'harxitflow' when AUTO_LOGIN is enabled."
    ),
    password: str = typer.Option(
        None, help="Password for the superuser. Defaults to 'harxitflow' when AUTO_LOGIN is enabled."
    ),
    log_level: str = typer.Option("error", help="Logging level.", envvar="HARXITFLOW_LOG_LEVEL"),
    auth_token: str = typer.Option(
        None, help="Authentication token of existing superuser.", envvar="HARXITFLOW_SUPERUSER_TOKEN"
    ),
) -> None:
    """Create a superuser.
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    if not settings_service.auth_settings.ENABLE_SUPERUSER_CLI:
        ...
        typer.echo("Set HARXITFLOW_ENABLE_SUPERUSER_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    from harxitflow.services.database.models.user.crud import get_all_superusers
    ...
    if settings_service.auth_settings.AUTO_LOGIN:
        if not is_first_setup:
            ...
            typer.echo("1. Set HARXITFLOW_AUTO_LOGIN=false")
    ...
    # AUTO_LOGIN is false - production mode
        ...
        if not auth_token:
            ...
            typer.echo("To get a token, use: `uv run harxitflow api_key`")
            raise typer.Exit(1)
    ...
    async with session_scope() as session:
        from harxitflow.services.deps import get_auth_service
...
# command to copy the harxitflow database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'harxitflow.db' and 'harxitflow-pre.db' files from the cache directory to the current
    directory.
    ...
    cache_dir = Path(user_cache_dir("harxitflow"))
    db_path = cache_dir / "harxitflow.db"
    pre_db_path = cache_dir / "harxitflow-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from harxitflow.services.database.models.api_key.crud import create_api_key, delete_api_key
...
def show_version(*, value: bool):
    if value:
        ...
        typer.echo(f"harxitflow {version}")
        raise typer.Exit
