...
from harxitflow.initial_setup.constants import (
...
from harxitflow.services.database.models.folder.constants import (
    DEFAULT_FOLDER_DESCRIPTION,
    DEFAULT_FOLDER_NAME,
    LEGACY_FOLDER_NAMES,
)
from harxitflow.services.database.models.folder.model import Folder, FolderCreate, FolderRead
...
async def get_or_create_assistant_folder(session, user_id: UUID):
    """Create or get the HarxitFlow Assistant folder for a specific user.
    ...
    Returns:
        The HarxitFlow Assistant folder
    """
...
async def create_or_update_agentic_flows(session: AsyncSession, user_id: UUID) -> None:
    """Create or update agentic flows in the HarxitFlow Assistant folder for a user.

    This function is called on user login to ensure that all agentic flows
    are present and up-to-date in the user's HarxitFlow Assistant folder.
    ...
    try:
        # Get or create the HarxitFlow Assistant folder
...
async def load_flows_from_directory() -> None:
    """On harxitflow startup, this loads all flows from the directory specified in the settings.
...
async def initialize_auto_login_default_superuser() -> None:
    ...
    async with session_scope() as async_session:
        ...
        from harxitflow.api.utils.mcp.agentic_mcp import initialize_agentic_user_variables
