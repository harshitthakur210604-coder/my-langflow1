"""HarxitFlow Assistant API router.

This module provides the HTTP endpoints for the HarxitFlow Assistant.
...
from harxitflow.agentic.api.schemas import AssistantRequest
from harxitflow.agentic.services.assistant_service import (
    execute_flow_with_validation,
    execute_flow_with_validation_streaming,
)
from harxitflow.agentic.services.flow_executor import execute_flow_file
...
from harxitflow.api.utils.core import CurrentActiveUser, DbSession
...
async def check_assistant_config(
    ...
    """Check if the HarxitFlow Assistant is properly configured.
...
async def assist(
    ...
    """Chat with the HarxitFlow Assistant."""
    ctx = await _resolve_assistant_context(request, current_user.id, session)

    logger.info(f"Executing {HARXITFLOW_ASSISTANT_FLOW} with {ctx.provider}/{ctx.model_name}")

    return await execute_flow_with_validation(
        flow_filename=HARXITFLOW_ASSISTANT_FLOW,
...
async def assist_stream(
    ...
    """Chat with the HarxitFlow Assistant with streaming progress updates."""
    ctx = await _resolve_assistant_context(request, current_user.id, session)
