...
from harxitflow.api.utils import CurrentActiveUser, DbSession
from harxitflow.api.v1.chat import build_flow_and_stream
from harxitflow.memory import aadd_messagetables
from harxitflow.schema.properties import Properties
from harxitflow.services.auth.utils import get_current_user_for_websocket
...
from harxitflow.services.deps import get_variable_service, session_scope
from harxitflow.utils.voice_utils import BYTES_PER_24K_FRAME, VAD_SAMPLE_RATE_16K, resample_24k_to_16k
...
async def authenticate_and_get_openai_key(session: DbSession, user: User, websocket: WebSocket):
    ...
    if user is None:
        await websocket.send_json(
            {
                "type": "error",
                "code": "harxitflow_auth",
                "message": "You must pass a valid HarxitFlow token or cookie.",
            }
        )
        return None, None
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "harxitflow.voice_mode.config":
                            await logger.ainfo(f"harxitflow.voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "harxitflow.elevenlabs.config":
                            await logger.ainfo(f"harxitflow.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "harxitflow.elevenlabs.config":
                            await logger.ainfo(f"harxitflow.elevenlabs.config {event}")
