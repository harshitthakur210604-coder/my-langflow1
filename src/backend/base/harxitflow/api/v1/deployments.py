...
from harxitflow.api.utils import CurrentActiveUser, DbSession, DbSessionReadOnly
from harxitflow.api.v1.mappers.deployments import get_deployment_mapper
from harxitflow.api.v1.mappers.deployments.helpers import (
...
from harxitflow.services.adapters.deployment.context import deployment_provider_scope
...
from harxitflow.services.database.models.deployment_provider_account.crud import (
...
DeploymentProviderAccountIdQuery = Annotated[
    UUID,
    Query(description="HarxitFlow DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentProviderAccountIdPath = Annotated[
    UUID,
    Path(description="HarxitFlow DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentIdPath = Annotated[
    UUID,
    Path(description="HarxitFlow DB deployment UUID (`deployment.id`)."),
]
...
DeploymentIdQuery = Annotated[
    UUID,
    Query(description="HarxitFlow DB deployment UUID (`deployment.id`)."),
]
...
IncludeProviderDeleteQuery = Annotated[
    ...
    Query(
        description=(
            ...
            "When false, only the local HarxitFlow DB row is removed; "
...
async def _delete_local_deployment_row_with_commit_retry(
    *,
    ...
    stale HarxitFlow row that still blocks later reads or provider-account
    deletion.
    """
    try:
...
async def create_deployment(
    ...
    if existing_resource_key is not None:
        ...
        if existing_deployment is not None:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"The agent '{existing_resource_key}' is already managed by HarxitFlow. "
...
async def list_deployments(
    ...
    *,
    load_from_provider: Annotated[
        bool,
        Query(
            description=("When true, list deployments directly from the provider (bypassing HarxitFlow deployment rows).")
        ),
    ] = False,
    flow_version_ids: Annotated[
        ...
        Query(
            description=(
                "Optional HarxitFlow flow version ids (pass as repeated query params, "
...
async def update_snapshot(
    ...
    *,
    ...
    ``provider_snapshot_id``.  Only HarxitFlow-tracked snapshots (those with
    a ``flow_version_deployment_attachment`` row) can be updated.
    """
    from harxitflow.services.database.models.deployment.crud import get_deployment as get_deployment_row
    from harxitflow.services.database.models.flow_version.crud import get_flow_version_entry
