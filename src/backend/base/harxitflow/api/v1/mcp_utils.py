...
from harxitflow.api.v1.endpoints import simple_run_flow
from harxitflow.api.v1.schemas import SimplifiedAPIRequest
from harxitflow.helpers.flow import json_schema_from_flow
...
# Carries per-request variables injected via HTTP headers (e.g., X-HarxitFlow-Global-Var-*)
            ...
            if project_id is None:
                ...
                for user_file in user_files:
                    ...
                    if stored_filename.startswith(f"{MCP_SERVERS_FILE}_{current_user.id}"):
                        # reserved file name for harxitflow MCP server config file(s)
                        continue
...
async def handle_read_resource(uri: str, project_id: UUID | str | None = None) -> bytes:
    ...
    try:
        ...
        # not enough because the filesystem doesn't know about HarxitFlow users, and
