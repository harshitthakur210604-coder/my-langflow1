...
from harxitflow.api.v1.mappers.deployments.contracts import CreateFlowArtifactProviderData
from harxitflow.api.v1.schemas.deployments import ValidatedUrl
from harxitflow.services.database.models.deployment_provider_account.utils import validate_provider_url
...
class WatsonxApiCreatedTool(BaseModel):
    """API response shape for a tool created from a HarxitFlow flow version."""
...
class _WatsonxApiAgentExecutionResultBase(BaseModel):
    ...
    ``provider_data``.  The enclosing response only carries HarxitFlow-owned
    fields (``deployment_id``).  ``deployment_id`` (HarxitFlow DB UUID) is
