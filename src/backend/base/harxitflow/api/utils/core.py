...
from harxitflow.services.auth.utils import get_current_active_user, get_current_active_user_mcp
...
from harxitflow.services.store.utils import get_lf_version_from_pypi
from harxitflow.utils.constants import HARXITFLOW_GLOBAL_VAR_HEADER_PREFIX

if TYPE_CHECKING:
    from harxitflow.services.store.schema import StoreComponentCreate
...
async def check_harxitflow_version(component: StoreComponentCreate) -> None:
    ...
    harxitflow_version = await get_lf_version_from_pypi()
    if harxitflow_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of HarxitFlow")
    if harxitflow_version != component.last_tested_version:
        await logger.awarning(
            f"Your version of HarxitFlow ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({harxitflow_version}) and try again."
        )
...
def extract_global_variables_from_headers(headers) -> dict[str, str]:
    """Extract global variables from HTTP headers with prefix X-HARXITFLOW-GLOBAL-VAR-*.
    ...
    Example:
        headers = {"X-HARXITFLOW-GLOBAL-VAR-API-KEY": "secret", "Content-Type": "application/json"}
    ...
    try:
        for header_name, header_value in headers.items():
            ...
            if header_lower.startswith(HARXITFLOW_GLOBAL_VAR_HEADER_PREFIX):
                var_name = header_lower[len(HARXITFLOW_GLOBAL_VAR_HEADER_PREFIX) :].upper()
