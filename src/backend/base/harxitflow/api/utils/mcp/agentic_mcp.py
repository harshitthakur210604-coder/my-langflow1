"""Utilities for auto-configuring the HarxitFlow Agentic MCP server."""
...
from harxitflow.api.v2.mcp import get_server_list, update_server
from harxitflow.services.database.models.user.model import User
from harxitflow.services.deps import get_service, get_variable_service
...
async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    """Auto-configure the HarxitFlow Agentic MCP server for all users.

    This function adds the harxitflow-agentic MCP server to each user's MCP
    ...
    await logger.ainfo("Auto-configuring HarxitFlow Agentic MCP server for all users...")
    ...
    try:
        ...
        server_name = "harxitflow-agentic"
        python_executable = sys.executable
        server_config = {
            "command": python_executable,
            "args": ["-m", "harxitflow.agentic.mcp"],
            "metadata": {
                "description": "HarxitFlow Agentic MCP server providing tools for flow/component operations, "
                "template search, and graph visualization",
                "auto_configured": True,
                "harxitflow_internal": True,
            },
        }
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the HarxitFlow Agentic MCP server from all users.

    This function removes the harxitflow-agentic MCP server from each user's MCP
    configuration. Used when agentic experience is disabled.

    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing HarxitFlow Agentic MCP server from all users...")
