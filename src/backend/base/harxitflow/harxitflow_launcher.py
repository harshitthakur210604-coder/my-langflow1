...
def main():
    """Launches harxitflow with appropriate environment setup.
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        from harxitflow.__main__ import main as harxitflow_main

        harxitflow_main()


def _launch_with_exec():
    """Launch harxitflow by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "harxitflow.__main__", *sys.argv[1:]])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec harxitflow: {e}", file=sys.stderr)
        sys.exit(1)
