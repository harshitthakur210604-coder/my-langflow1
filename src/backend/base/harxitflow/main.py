...
from harxitflow.api import health_check_router, log_router
from harxitflow.api.router import router
from harxitflow.api.v1.mcp_projects import init_mcp_servers
...
from harxitflow.middleware import ContentSizeLimitMiddleware
from harxitflow.plugin_routes import load_plugin_routes
from harxitflow.services.database.models.deployment.exceptions import DeploymentGuardError
from harxitflow.services.database.service import UnsupportedPostgreSQLVersionError
from harxitflow.services.deps import (
    get_queue_service,
    get_service,
...
from harxitflow.services.utils import initialize_services, initialize_settings_service, teardown_services
from harxitflow.utils.mcp_cleanup import cleanup_mcp_sessions
...
def warn_about_future_cors_changes(settings):
    ...
    if using_defaults:
        logger.warning(
            ...
            "Set HARXITFLOW_CORS_ORIGINS for production. Stricter defaults in v2.0."
        )
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            await logger.adebug(f"Starting HarxitFlow v{version}...")
        else:
            await logger.adebug("Starting HarxitFlow...")
        ...
        try:
            ...
            lock_file = Path(tempfile.gettempdir()) / "harxitflow_starter_projects.lock"
            ...
            if get_settings_service().settings.agentic_experience:
                from harxitflow.api.utils.mcp.agentic_mcp import initialize_agentic_global_variables
            ...
            if get_settings_service().settings.agentic_experience:
                from harxitflow.api.utils.mcp.agentic_mcp import auto_configure_agentic_mcp_server
            ...
            from harxitflow.api.v1.mcp import start_streamable_http_manager
            from harxitflow.api.v1.mcp_projects import start_project_task_group
            ...
            if "harxitflow migration --fix" not in str(exc):
            ...
            from harxitflow.__main__ import get_number_of_workers
            from harxitflow.cli.progress import create_harxitflow_shutdown_progress

            log_level = os.getenv("HARXITFLOW_LOG_LEVEL", "info").lower()
            num_workers = get_number_of_workers(get_settings_service().settings.workers)
            shutdown_progress = create_harxitflow_shutdown_progress(
            ...
            try:
                ...
                with shutdown_progress.step(1):
                    from harxitflow.api.v1.mcp import stop_streamable_http_manager
                    from harxitflow.api.v1.mcp_projects import stop_project_task_group
                ...
                with shutdown_progress.step(4):
                    await logger.adebug("HarxitFlow shutdown complete")
...
def create_app():
    ...
    if prome_port_str := os.environ.get("HARXITFLOW_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from harxitflow.api.v1 import mcp_router
    ...
    # Discover and register additional routers from plugins (harxitflow.plugins entry-point)
...
def get_static_files_dir():
    """Get the static files directory relative to HarxitFlow's main.py file."""
