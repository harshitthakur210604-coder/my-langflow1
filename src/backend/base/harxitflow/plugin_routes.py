...
Plugins register via the ``harxitflow.plugins`` entry-point group. They receive
a wrapper so they cannot overwrite or shadow existing HarxitFlow routes.
"""
...
def _get_route_keys(app: FastAPI) -> set[tuple[str, str]]:
    ...
    routes cannot overwrite or shadow existing HarxitFlow routes.
    """
...
def load_plugin_routes(app: FastAPI) -> None:
    ...
    Plugins register themselves via the ``harxitflow.plugins`` entry-point group.
    ...
    eps = entry_points(group="harxitflow.plugins")
