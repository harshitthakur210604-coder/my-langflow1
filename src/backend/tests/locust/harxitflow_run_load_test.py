#!/usr/bin/env python3
"""HarxitFlow Load Test Runner

This script provides an easy way to run HarxitFlow load tests.
For first-time setup, use setup_harxitflow_test.py to create test credentials.

Usage:
    # First time setup (run once):
    python setup_harxitflow_test.py --interactive
...
def check_harxitflow_running(host):
    """Check if HarxitFlow is already running."""
    try:
        import httpx
...
def wait_for_harxitflow(host, timeout=60):
    """Wait for HarxitFlow to be ready."""
    print(f"Waiting for HarxitFlow to be ready at {host}...")
    start_time = time.time()

    while time.time() - start_time < timeout:
        if check_harxitflow_running(host):
            print("✅ HarxitFlow is ready!")
            return True
        time.sleep(2)

    print(f"❌ HarxitFlow did not start within {timeout} seconds")
    return False


def start_harxitflow(host, port):
    """Start HarxitFlow server if not already running."""
    if check_harxitflow_running(host):
        print(f"✅ HarxitFlow is already running at {host}")
        return None

    print(f"Starting HarxitFlow server on port {port}...")

    # Start HarxitFlow in the background
    cmd = [
        sys.executable,
        "-m",
        "harxitflow",
    ...
    if wait_for_harxitflow(host, timeout=60):
        return process
    process.terminate()
    return None
...
def run_locust_test(args):
    ...
    locust_file = Path(__file__).parent / "harxitflow_locustfile.py"

    # Check for required environment variables
    if not os.getenv("API_KEY"):
        print("❌ API_KEY environment variable not found!")
        print("Run harxitflow_setup_test.py first to create test credentials.")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run HarxitFlow load tests with automatic setup",
...
Examples:
  ...
  # Run against existing HarxitFlow instance
  python run_load_test.py --host http://localhost:8000 --no-start-harxitflow
  ...
  python run_load_test.py --headless --csv results --users 25 --duration 60
    ...
    parser.add_argument(
        ...
        help="HarxitFlow host URL (default: http://localhost:7860, use https:// for remote instances)",
    )
    parser.add_argument("--port", type=int, default=7860, help="Port to start HarxitFlow on (default: 7860)")
    parser.add_argument(
        "--no-start-harxitflow",
        action="store_true",
        help="Don't start HarxitFlow automatically (assume it's already running)",
    )
    ...
    harxitflow_process = None

    try:
        # Start HarxitFlow if needed
        if not args.no_start_harxitflow:
            if args.host.startswith("https://") or not args.host.startswith("http://localhost"):
                print(f"⚠️  Remote host detected: {args.host}")
                print("   For remote instances, use --no-start-harxitflow flag")
                print("   Example: --host https://your-remote-instance.com --no-start-harxitflow")
                sys.exit(1)

            harxitflow_process = start_harxitflow(args.host, args.port)
            if not harxitflow_process:
                print("❌ Failed to start HarxitFlow")
                sys.exit(1)
        # Just check if it's running
        elif not check_harxitflow_running(args.host):
            print(f"❌ HarxitFlow is not running at {args.host}")
            if args.host.startswith("https://"):
                print("   Make sure your remote HarxitFlow instance is accessible")
            else:
                print("Either start HarxitFlow manually or remove --no-start-harxitflow flag")
            sys.exit(1)
        else:
            print(f"🔗 Using existing HarxitFlow instance at {args.host}")
            if args.host.startswith("https://"):
        ...
        # Clean up HarxitFlow process
        if harxitflow_process:
            print("\nStopping HarxitFlow server...")
            harxitflow_process.terminate()
            try:
                harxitflow_process.wait(timeout=10)
            except subprocess.TimeoutExpired:
                harxitflow_process.kill()
            print("✅ HarxitFlow server stopped")
