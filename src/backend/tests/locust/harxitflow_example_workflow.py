#!/usr/bin/env python3
"""Example HarxitFlow Load Testing Workflow

This script demonstrates the complete workflow for setting up and running
HarxitFlow load tests with real starter project flows.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Example HarxitFlow Load Testing Workflow")
    parser.add_argument("--auto", action="store_true", help="Run automatically without user input prompts")
    args = parser.parse_args()

    print("🚀 HarxitFlow Load Testing Example Workflow")
    ...
    setup_script = script_dir / "harxitflow_setup_test.py"
    runner_script = script_dir / "harxitflow_run_load_test.py"
    ...
    try:
        ...
        try:
            ...
            env = os.environ.copy()
            env["HARXITFLOW_HOST"] = creds["host"]
            env["API_KEY"] = creds["api_key"]
            env["FLOW_ID"] = creds["flow_id"]

            print("   🔧 Setting environment variables:")
            print(f"      HARXITFLOW_HOST={creds['host']}")
        ...
        success = run_command_with_env(
            [
                ...
                "--no-start-harxitflow",
                "--html",
                "harxitflow_load_test_report.html",
                "--csv",
                "harxitflow_load_test_results",
            ],
        ...
        print("   python setup_harxitflow_test.py --interactive")
        print()
        ...
        print("   - harxitflow_load_test_report.html (detailed HTML report)")
        print("   - harxitflow_load_test_results_*.csv (CSV data files)")
        print("   - harxitflow_load_test_detailed_errors_*.log (detailed error logs)")
        print("   - harxitflow_load_test_error_summary_*.json (error analysis)")
        print("   - harxitflow_server_logs_during_test_*.log (HarxitFlow server logs)")
        print()
        print("🧹 Clean up:")
        print("   - Remove test flows from HarxitFlow UI")
        ...
        if args.auto:
            ...
            print("   - harxitflow_load_test_report.html")
            print("   - harxitflow_load_test_results_*.csv")
        else:
            ...
            if cleanup_response == "y":
                files_to_clean = [
                    "example_test_creds.json",
                    "harxitflow_load_test_report.html",
                    "harxitflow_load_test_results_failures.csv",
                    "harxitflow_load_test_results_stats.csv",
                    "harxitflow_load_test_results_stats_history.csv",
                    "harxitflow_load_test_results_exceptions.csv",
                ]

                # Also clean up error logs (they have timestamps, so use glob pattern)
                import glob

                error_files = glob.glob("harxitflow_load_test_detailed_errors_*.log")
                error_files.extend(glob.glob("harxitflow_load_test_error_summary_*.json"))
                error_files.extend(glob.glob("harxitflow_server_logs_during_test_*.log"))
        ...
        print("You're now ready to use HarxitFlow load testing for your own projects.")
        print()
        print("📊 View your load test results:")
        print("   • Open harxitflow_load_test_report.html in your browser for detailed analysis")
        print("   • Check harxitflow_load_test_results_*.csv for raw data")
        print("   • Review harxitflow_load_test_detailed_errors_*.log for comprehensive error details")
        print("   • Analyze harxitflow_load_test_error_summary_*.json for error patterns")
        print("   • Examine harxitflow_server_logs_during_test_*.log for server-side issues")
