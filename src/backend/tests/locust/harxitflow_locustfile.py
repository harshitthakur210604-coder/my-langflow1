"""HarxitFlow API Locust Load Testing File.

Comprehensive load testing for HarxitFlow API with multiple user behaviors and performance analysis.
...
Environment Variables:
    - HARXITFLOW_HOST: Base URL for the HarxitFlow server (default: http://localhost:7860)
...
TEST_MESSAGES = {
    ...
    "realistic": "Hey, Could you check https://docs.harxitflow.org for me? Later, could you calculate 1390 / 192 ?",
}
...
ERROR_LOG_FILE = None
HARXITFLOW_LOG_FILE = None
DETAILED_ERRORS = []


def setup_error_logging():
    """Set up detailed error logging for the load test."""
    global ERROR_LOG_FILE, HARXITFLOW_LOG_FILE

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # Create detailed error log
    ERROR_LOG_FILE = f"harxitflow_load_test_detailed_errors_{timestamp}.log"

    # Set up error logger
    error_logger = logging.getLogger("harxitflow_load_test_errors")
    ...
    # Try to capture HarxitFlow logs
    harxitflow_log_paths = ["harxitflow.log", "logs/harxitflow.log", "../../../harxitflow.log", "../../../../harxitflow.log"]

    for log_path in harxitflow_log_paths:
        if Path(log_path).exists():
            HARXITFLOW_LOG_FILE = log_path
            break

    print("📝 Error logging setup:")
    print(f"   • Detailed errors: {ERROR_LOG_FILE}")
    if HARXITFLOW_LOG_FILE:
        print(f"   • HarxitFlow logs: {HARXITFLOW_LOG_FILE}")
    else:
        print("   • HarxitFlow logs: Not found (will monitor common locations)")
...
def save_error_summary():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    summary_file = f"harxitflow_load_test_error_summary_{timestamp}.json"
...
def capture_harxitflow_logs():
    """Capture recent HarxitFlow logs if available."""
    if not HARXITFLOW_LOG_FILE or not Path(HARXITFLOW_LOG_FILE).exists():
        return None

    try:
        # Read last 1000 lines of HarxitFlow log
        with open(HARXITFLOW_LOG_FILE) as f:
            lines = f.readlines()
            recent_lines = lines[-1000:] if len(lines) > 1000 else lines

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        captured_log_file = f"harxitflow_server_logs_during_test_{timestamp}.log"

        with open(captured_log_file, "w") as f:
            f.write("# HarxitFlow server logs captured during load test\n")
            f.write(f"# Original log file: {HARXITFLOW_LOG_FILE}\n")
            f.write(f"# Capture time: {datetime.now().isoformat()}\n")
            f.write(f"# Lines captured: {len(recent_lines)}\n\n")
            f.writelines(recent_lines)

        print(f"📋 HarxitFlow logs captured: {captured_log_file}")
        return captured_log_file

    except Exception as e:
        print(f"⚠️  Could not capture HarxitFlow logs: {e}")
        return None
...
def on_test_stop(environment, **_kwargs):
    ...
    print(f"\n{'=' * 60}")
    print(f"HARXITFLOW API LOAD TEST RESULTS - GRADE: {grade}")
    print(f"{'=' * 60}")
    ...
    # Capture HarxitFlow logs
    capture_harxitflow_logs()
...
class BaseHarxitFlowUser(FastHttpUser):
    """Base class for all HarxitFlow API load testing user types."""

    abstract = True
    REQUEST_TIMEOUT = float(os.getenv("REQUEST_TIMEOUT", "30.0"))

    # Use the host provided by environment variable or default
    host = os.getenv("HARXITFLOW_HOST", "http://localhost:7860")
    ...
    def on_start(self):
        ...
        if not self.api_key:
            raise ValueError("API_KEY environment variable is required. Run setup_harxitflow_test.py first.")

        if not self.flow_id:
            raise ValueError("FLOW_ID environment variable is required. Run setup_harxitflow_test.py first.")
    ...
    def make_request(self, message_type="simple", tag_suffix=""):
        ...
        # HarxitFlow API payload structure
        payload = {
            "input_value": message,
            "output_type": "chat",
            "input_type": "chat",
        ...
        try:
            with self.client.post(
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        # HarxitFlow API success check - look for outputs
                        if data.get("outputs"):
...
class NormalUser(BaseHarxitFlowUser):
...
class AggressiveUser(BaseHarxitFlowUser):
...
class SustainedLoadUser(BaseHarxitFlowUser):
...
class TailLatencyHunter(BaseHarxitFlowUser):
...
class ScalabilityTestUser(BaseHarxitFlowUser):
...
class BurstUser(BaseHarxitFlowUser):
