"""Unit tests for harxitflow_sdk.testing and RunResponse helpers.

Uses respx to mock HTTP so no live HarxitFlow instance is needed.
"""
...
from harxitflow_sdk.client import AsyncHarxitFlowClient, HarxitFlowClient
from harxitflow_sdk.models import RunOutput, RunResponse
from harxitflow_sdk.testing import AsyncFlowRunner, FlowRunner, pytest_addoption
...
# Minimal RunResponse payload matching the standard HarxitFlow chat shape
_CHAT_RUN_RESPONSE = {
    "session_id": "sess-abc",
    "outputs": [
        {
...
def _sync_client() -> HarxitFlowClient:
    return HarxitFlowClient(base_url=_BASE, api_key="test-key")  # pragma: allowlist secret


def _async_client() -> AsyncHarxitFlowClient:
    return AsyncHarxitFlowClient(base_url=_BASE, api_key="test-key")  # pragma: allowlist secret
...
def test_flow_runner_and_async_runner_are_importable():
    """FlowRunner and AsyncFlowRunner can be imported from harxitflow_sdk.testing."""
    from harxitflow_sdk.testing import AsyncFlowRunner as ImportedAsync
    from harxitflow_sdk.testing import FlowRunner as ImportedSync
...
def test_pytest_addoption_registers_options(pytestconfig):
    """The plugin registers all --harxitflow-* options (verified via public getoption API)."""
