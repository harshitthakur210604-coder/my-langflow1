"""Environment configuration for harxitflow-sdk.

Loads named environment definitions from a TOML file so teams can switch
between HarxitFlow instances (dev / staging / production) without code changes.

Config file lookup order
------------------------
1. Path given explicitly to ``load_environments()`` or ``get_client()``.
2. The ``HARXITFLOW_ENVIRONMENTS_FILE`` environment variable.
3. ``harxitflow-environments.toml`` in the current working directory.
4. ``~/.config/harxitflow/environments.toml``
...
.. code-block:: toml
    ...
    api_key_env = "HARXITFLOW_STAGING_API_KEY"   # env-var that holds the key  # pragma: allowlist secret

    [environments.production]
    url = "https://harxitflow.example.com"
    api_key_env = "HARXITFLOW_PROD_API_KEY"  # pragma: allowlist secret
...
from harxitflow_sdk.exceptions import EnvironmentConfigError, EnvironmentNotFoundError
...
_ENV_VAR = "HARXITFLOW_ENVIRONMENTS_FILE"
_LOCAL_NAME = "harxitflow-environments.toml"
_USER_PATH = Path.home() / ".config" / "harxitflow" / "environments.toml"

_EXAMPLE_CONFIG = """\
# harxitflow-environments.toml
#
# Define named HarxitFlow environments.  The api_key_env field is the *name*
# of an environment variable that holds the API key for that instance.
#
# [environments.staging]
# url = "https://staging.harxitflow.example.com"
# api_key_env = "HARXITFLOW_STAGING_API_KEY"  # pragma: allowlist secret
#
# [environments.production]
# url = "https://harxitflow.example.com"
# api_key_env = "HARXITFLOW_PROD_API_KEY"  # pragma: allowlist secret
...
def load_environments(
    ...
    ----------
    config_file:
        Explicit path to a ``harxitflow-environments.toml`` file. If omitted,
    ...
    if file_path is None:
        raise EnvironmentConfigError(
            "No harxitflow-environments.toml found. "
...
def get_client(
    ...
    *,
    ...
    Example::
        ...
        from harxitflow_sdk import get_client
...
def get_async_client(
    ...
    *,
    ...
    Example::
        ...
        from harxitflow_sdk import get_async_client
