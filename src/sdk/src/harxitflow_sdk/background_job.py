...
from harxitflow-ai/sdk PR #1 (Janardan Singh Kavia, IBM Corp., Apache 2.0)
adapted for the HarxitFlow V1 ``/api/v1/run/{id}`` endpoint.
...
class BackgroundJob:
    """Non-blocking handle for an in-flight :meth:`AsyncHarxitFlowClient.run` call.

    Returned by :meth:`AsyncHarxitFlowClient.run_background`.  The underlying
        ...
        *,
        ...
        Args:
            timeout: Maximum seconds to wait.  ``None`` (default) means wait
                     indefinitely.  Raises :exc:`HarxitFlowTimeoutError` on
                     expiry.
        ...
        Raises:
            HarxitFlowTimeoutError: If *timeout* elapses before the run finishes.
        ...
        try:
            ...
            raise HarxitFlowTimeoutError(msg) from exc
