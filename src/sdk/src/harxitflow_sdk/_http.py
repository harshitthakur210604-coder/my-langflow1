...
_logger = logging.getLogger("harxitflow_sdk.client")
...
def _raise_for_status_code(status: int, detail: str) -> None:
    ...
    if status in (HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN):
        raise HarxitFlowAuthError(status, detail)
    if status == HTTPStatus.NOT_FOUND:
        raise HarxitFlowNotFoundError(status, detail)
    if status == HTTPStatus.UNPROCESSABLE_ENTITY:
        raise HarxitFlowValidationError(status, detail)
    raise HarxitFlowHTTPError(status, detail)
...
def _connection_error(base_url: str, exc: Exception) -> HarxitFlowConnectionError:
    msg = f"Could not connect to HarxitFlow at {base_url}: {exc}"
    return HarxitFlowConnectionError(msg)
