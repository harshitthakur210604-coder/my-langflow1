"""Async HTTP client for the HarxitFlow REST API.

Preferred usage via the short alias::

    from harxitflow_sdk import AsyncClient

    async with AsyncClient("https://harxitflow.example.com", api_key="...") as client:
...
from harxitflow_sdk._client_common import _ClientCommon
from harxitflow_sdk._http import (
    _DEFAULT_TIMEOUT,
    _build_headers,
    _connection_error,
    _logger,
...
class AsyncHarxitFlowClient(_ClientCommon):
    """Async client for the HarxitFlow REST API.
    ...
    async def run_background(
        ...
        *,
        ...
        Args:
            ...
            input_type: HarxitFlow input type (default ``"chat"``).
            output_type: HarxitFlow output type (default ``"chat"``).
            tweaks: Optional component tweaks dict.

        Returns:
            A :class:`BackgroundJob` wrapping the in-flight asyncio task.

        Adapted from ``BackgroundJob`` in harxitflow-ai/sdk PR #1
    ...
    async def pull_project(
        ...
        *,
        ...
        .. note::
            ...
            :meth:`HarxitFlowClient.pull_project` for details.
...
#: Short alias for :class:`AsyncHarxitFlowClient`.
#:
#: Example::
#:
#:     from harxitflow_sdk import AsyncClient
#:     async with AsyncClient("https://harxitflow.example.com", api_key="...") as c:
#:         flows = await c.list_flows()
AsyncClient = AsyncHarxitFlowClient
