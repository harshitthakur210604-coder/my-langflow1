"""Sync HTTP client for the HarxitFlow REST API.
...
The async counterpart lives in :mod:`harxitflow_sdk._async_client`.
"""
...
# Re-export async client so that existing ``from harxitflow_sdk.client import ...``
...
class HarxitFlowClient(_ClientCommon):
    """Synchronous client for the HarxitFlow REST API.
...
#: Short alias for :class:`HarxitFlowClient`.
#:
#: Example::
#:
#:     from harxitflow_sdk import Client
...
#:     flows  = client.list_flows()
#:     result = client.run_flow("my-endpoint", RunRequest(input_value="Hello"))
Client = HarxitFlowClient

__all__ = ["AsyncClient", "AsyncHarxitFlowClient", "Client", "HarxitFlowClient"]
