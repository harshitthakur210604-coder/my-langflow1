...
- **API hosts (for example HarxitFlow) own API-facing payload models**:
...
- HarxitFlow defines deployment API registry (`DeploymentApiPayloads`) and mapper behavior
...
```toml
[services]
database_service = "harxitflow.services.database.service:DatabaseService"
storage_service = "harxitflow.services.storage.local:LocalStorageService"
cache_service = "harxitflow.services.cache.service:ThreadingInMemoryCache"
```
...
4. Enable debug logging: `export HARXITFLOW_LOG_LEVEL=DEBUG`

### Import Errors

**Error:** `ModuleNotFoundError: No module named 'harxitflow'`

**Solutions:**
1. Ensure harxitflow is installed: `pip install harxitflow`
2. Check if module path in config is correct
3. Verify the package is importable: `python -c "import harxitflow.services.database"`
...
See:
- `lfx.toml.example` - Example configuration file showing HarxitFlow service registration
- `src/lfx/services/` - Minimal built-in service implementations (auth, telemetry, tracing, variable, storage, etc.)
  - Auth: `lfx.services.auth.base` (BaseAuthService) and `lfx.services.auth.service` (AuthService). Use `get_auth_service()` from `lfx.services.deps`. Override with `auth_service = "harxitflow.services.auth.service:AuthService"` in config for full JWT/API key auth.
- `src/backend/base/harxitflow/services/` - Full-featured HarxitFlow services
