"""lfx login -- validate credentials against a remote HarxitFlow instance.
...
Validate inline credentials::
    ...
    lfx login --target https://harxitflow.example.com --api-key sk-abc123
...
def _probe_connection(client: Any, sdk: Any) -> tuple[bool, str, int]:
    ...
    try:
    ...
    except sdk.HarxitFlowAuthError:
        return False, "auth", 0
    except sdk.HarxitFlowConnectionError as exc:
        ...
        # local HarxitFlow version exactly.  Treat this as a successful probe.
    ...
    *,
    ...
    if env_cfg.api_key is None:
        ...
        if key_env_name:
            ...
            warning_parts.append("  Add [bold]api_key_env: HARXITFLOW_<ENV>_API_KEY[/bold] to your config,")
    ...
    if not ok:
        if msg == "auth":
            ...
            if key_env_name:
                err_console.print("  1. Open HarxitFlow → Settings → API Keys → Create a new key")
        ...
        if msg.startswith("connection:"):
            ...
            err_console.print()
            err_console.print("[bold]How to fix:[/bold]")
            err_console.print("  • Make sure your HarxitFlow instance is running")
            err_console.print("  • Check the URL in your .lfx/environments.yaml")
            err_console.print("  • If running locally: [bold]harxitflow run[/bold] or [bold]lfx serve <flow.json>[/bold]")
            raise typer.Exit(1)
    ...
    elif os.environ.get("HARXITFLOW_API_KEY") == env_cfg.api_key and env_cfg.api_key:
        key_source = "  [dim]from HARXITFLOW_API_KEY[/dim]"
    ...
    if env_cfg.name in ("__inline__", "__env__") and env_cfg.api_key:
        ...
        console.print(f"[dim]      api_key_env: HARXITFLOW_{env_display.upper()}_API_KEY[/dim]")
