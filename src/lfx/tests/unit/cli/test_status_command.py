...
class _FakeHarxitFlowNotFoundError(Exception):
    """Stand-in for harxitflow_sdk.exceptions.HarxitFlowNotFoundError in unit tests."""
...
def _fake_remote_flow(flow_id: UUID = _FLOW_ID, flow_dict: dict | None = None) -> MagicMock:
    """Return a MagicMock that looks like a harxitflow_sdk Flow model."""
    remote = MagicMock()
    remote.id = flow_id
...
def _make_sdk_triple(
    ...
    return _identity_normalize, _json_flow_to_json, client_cls, _FakeHarxitFlowNotFoundError
...
class TestStatusCommandSynced:
    ...
    def test_client_is_closed_after_requests_finish(self, tmp_path):
        ...
        triple = _identity_normalize, _json_flow_to_json, MagicMock(return_value=client), _FakeHarxitFlowNotFoundError
...
class TestStatusCommandNew:
    def test_not_found_on_remote_gives_new_status(self, tmp_path):
        """Remote raises HarxitFlowNotFoundError → status 'new' → exits 1."""
        p = _write_flow(tmp_path, "flow.json")
        client = _make_client_mock(get_flow_side_effect=_FakeHarxitFlowNotFoundError("not found"))
        triple = _make_sdk_triple(client)
