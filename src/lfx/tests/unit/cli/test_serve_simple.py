...
# Tests that invoke `serve` with valid-looking input may hang while harxitflow
...
def test_serve_command_invalid_json():
    ...
    with patch.dict(os.environ, {"HARXITFLOW_API_KEY": "test-key"}):  # pragma: allowlist secret
        runner = CliRunner()
...
def test_serve_command_missing_api_key():
    """Serve command must exit 1 and mention HARXITFLOW_API_KEY when no key is set."""
    from lfx.__main__ import app
    ...
    try:
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert result.exit_code == 1
            assert "HARXITFLOW_API_KEY" in str(result.output or result.exception or "")
    finally:
...
def test_serve_command_with_flow_json():
    ...
    env = {"HARXITFLOW_API_KEY": "test-key"}  # pragma: allowlist secret
