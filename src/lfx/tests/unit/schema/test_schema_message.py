...
@pytest.fixture
def harxitflow_cache_dir(tmp_path):
    """Create a temporary harxitflow cache directory."""
    cache_dir = tmp_path / "harxitflow"
    cache_dir.mkdir(parents=True)
    return cache_dir


@pytest.fixture
def sample_image(harxitflow_cache_dir):
    """Create a sample image file for testing."""
    # Create the test_flow directory in the cache
    flow_dir = harxitflow_cache_dir / "test_flow"
    ...
    real_cache_dir = Path(user_cache_dir("harxitflow"))
...
def test_message_prompt_serialization():
    template = "Hello, {name}!"
    message = Message.from_template(template, name="HarxitFlow")
    assert message.text == "Hello, HarxitFlow!"
...
def test_message_with_multiple_images(sample_image, harxitflow_cache_dir):
    ...
    real_cache_dir = Path(user_cache_dir("harxitflow")) / "test_flow"
    real_cache_dir.mkdir(parents=True, exist_ok=True)
