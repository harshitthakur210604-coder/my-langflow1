...
"""
        ...
        # Verify dependency analysis results
        dep_info = mock_frontend.metadata["dependencies"]
        assert dep_info["total_dependencies"] == 1  # Only harxitflow (os is stdlib, filtered out)

        # Check for dependencies
        package_names = [pkg["name"] for pkg in dep_info["dependencies"]]
        assert "harxitflow" in package_names  # harxitflow should be detected as external
...
from harxitflow.base.models.model import LCModelComponent
...
"""
        ...
        build_component_metadata(
            mock_frontend, test_component, "harxitflow.components.lmstudio", "LMStudioModelComponent"
        )
        ...
        assert "harxitflow" in package_names  # project dependency
