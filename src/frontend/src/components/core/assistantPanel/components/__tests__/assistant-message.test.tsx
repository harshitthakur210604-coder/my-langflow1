...
// Desktop HarxitFlow Assistant avatar bug is fixed: the component must render
...
jest.mock("@/assets/harxitflow_assistant.svg", () => "harxitflow-icon.svg");
...
describe("AssistantMessageItem", () => {
  describe("user messages", () => {
    ...
    // Regression guard: HarxitFlow Desktop shipped with a broken user avatar in
  ...
  describe("assistant messages", () => {
    it("should render assistant label with HarxitFlow icon", () => {
      const message = createMessage({
        role: "assistant",
      ...
      expect(screen.getByText("HarxitFlow Assistant")).toBeInTheDocument();
      expect(screen.getByAltText("HarxitFlow Assistant")).toBeInTheDocument();
    });
  });
  ...
  describe("plain text response", () => {
    it("should render markdown for regular text content", () => {
      const message = createMessage({
        role: "assistant",
        content: "HarxitFlow is a visual flow builder.",
        status: "complete",
      });
      ...
      expect(screen.getByTestId("markdown-content")).toHaveTextContent(
        "HarxitFlow is a visual flow builder.",
      );
    });
    ...
    it("should_render_markdown_when_qa_response_contains_example_component_code", () => {
      ...
      const qaWithExampleCode = [
        ...
        "```\n\n",
        "Save the file and restart HarxitFlow.",
      ].join("");
