...
def update_uv_dep(base_version: str) -> None:
    ...
    # For the main project, update the harxitflow-base dependency in the UV section
    # Updated pattern to handle PEP 440 version suffixes, extras (e.g., [complete]),
    # and both ~= and == version specifiers
    # Also handles both harxitflow-base and harxitflow-base-nightly names
    # Captures extras in group 3 to preserve them in the replacement
    pattern = re.compile(
        r'(dependencies\s*=\s*\[\s*\n\s*)("harxitflow-base(?:-nightly)?((?:\[[^\]]+\])?)(?:~=|==|>=)[\d.]+(?:\.(?:post|dev|a|b|rc)\d+)*")'
    )
    ...
    replacement = rf'\1"harxitflow-base-nightly{extras}=={base_version}"'
