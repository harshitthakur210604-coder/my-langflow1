# Deploy HarxitFlow on Google Cloud Platform
...
In this tutorial, you will learn how to deploy HarxitFlow on [Google Cloud Platform](https://cloud.google.com/) (GCP) using Google Cloud Shell.
...
- A project on GCP where you want to deploy HarxitFlow
...
In the next step, you'll configure the GCP environment and deploy HarxitFlow.

## Configure the GCP environment and deploy HarxitFlow
Run the deploy_harxitflow_gcp_spot.sh script to configure the GCP environment and deploy HarxitFlow:

```sh
gcloud config set project <walkthrough-project-id/>
bash ./deploy_harxitflow_gcp_spot.sh
```

The script will:

1. Check if the required resources (VPC, subnet, firewall rules, and Cloud Router) exist and create them if needed
2. Create a startup script to install Python, HarxitFlow, and Nginx
3. Create a Compute Engine VM instance with the specified configuration and startup script
4. Run HarxitFlow to serve content on TCP port 7860
...
In the next step, you'll learn how to connect to the HarxitFlow VM.

## Connect to the HarxitFlow VM
To connect to your new HarxitFlow VM, follow these steps:
...
<br>**or**
3. Run the following command to display the URL for your HarxitFlow environment:
```bash
export HARXITFLOW_IP=$(gcloud compute instances list --filter="NAME=harxitflow-dev" --format="value(EXTERNAL_IP)")

echo http://$HARXITFLOW_IP:7860
```

4. Click on the HarxitFlow URL in cloudshell to be greeted by the HarxitFlow Dev environment

Congratulations! You have successfully deployed HarxitFlow on Google Cloud Platform.
...
```sql
gcloud compute instances delete harxitflow-dev --zone us-central1-a --quiet
```
