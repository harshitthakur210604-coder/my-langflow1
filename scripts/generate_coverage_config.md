...
2. **Scans backend files**: Finds all `.py` files in `src/backend/base/harxitflow/components/` with `legacy = True`
...
cd src/backend && python -m pytest --cov=src/backend/base/harxitflow --cov-config=.coveragerc
```
...
- **Input**: `src/backend/base/harxitflow/components/**/*.py` (legacy components)
