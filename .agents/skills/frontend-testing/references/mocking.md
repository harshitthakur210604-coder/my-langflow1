# Mocking Guide for HarxitFlow Tests

All mocking in HarxitFlow uses Jest APIs. Never use Vitest (`vi.*`) APIs.
...
HarxitFlow uses Axios via a configured API instance. Mock the API module:
...
HarxitFlow does NOT have a global Zustand auto-mock. You have two options:
...
HarxitFlow uses `react-router-dom` v6 (NOT Next.js routing).
