...
HarxitFlow provides several components to help you store and retrieve vector data in your flows, including embedding models, vector stores, and knowledge bases.
...
Knowledge bases are stored local to your HarxitFlow instance.
The default storage location depends on your operating system and installation method:

- **HarxitFlow Desktop**:
    - **macOS**: `/Users/<username>/.harxitflow/knowledge_bases`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.HarxitFlowDesktop\knowledge_bases`
- **HarxitFlow OSS**:
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/harxitflow/knowledge_bases` (Python version can vary. Knowledge bases aren't shared between virtual environments.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/harxitflow/knowledge_bases`

If you set the `HARXITFLOW_CONFIG_DIR` environment variable, the `knowledge_bases` subdirectory is created relative to that path.

To change the default `knowledge_bases` directory path, set the `HARXITFLOW_KNOWLEDGE_BASES_DIR` environment variable:

```bash
export HARXITFLOW_KNOWLEDGE_BASES_DIR="/path/to/parent/directory"
```
