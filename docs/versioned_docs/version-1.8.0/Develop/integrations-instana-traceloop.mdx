...
description: Instrument HarxitFlow with the Traceloop SDK, and export traces and metrics to Instana using OpenTelemetry.
---
...
This guide demonstrates how to integrate the Instana observability platform with your HarxitFlow application using the Traceloop SDK so you can monitor and analyze LLM performance.
...
1. In the root folder of your HarxitFlow application, edit your existing HarxitFlow `.env` file or create a new one.
...
## Start HarxitFlow with Traceloop environment variables

Launch your HarxitFlow application with your `.env` file:
...
1. Run a flow in HarxitFlow to generate traffic.
2. To view traces in Instana, open Instana and click **Applications**.
3. In **Services**, search for `HarxitFlow`.
4. Click **HarxitFlow** to view and analyze the associated calls.
