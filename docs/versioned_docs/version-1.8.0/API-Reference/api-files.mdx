...
Use the `/files` endpoints to move files between your local machine and HarxitFlow.

All `/files` endpoints (both `/v1/files` and `/v2/files`) require authentication with a HarxitFlow API key.
...
- `/v2` files are organized by `user_id` instead of `flow_id`.
  ...
  You can upload a file to HarxitFlow one time, and use it with multiple flows.
- `/v2` files are tracked in the HarxitFlow database.
...
Send image files to HarxitFlow to use them in flows.

The default file limit is 1024 MB.
To change this limit, set the `HARXITFLOW_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).
...
1. Attach the image to a `POST /v1/files/upload/$FLOW_ID` request with `--form` (`-F`) and the file path:
    ...
    A successful request returns the `file_path` for the image in the HarxitFlow file management system in the format `FLOW_ID/TIMESTAMP_FILENAME.TYPE`.
    For example:
...
To create a HarxitFlow API key and export it as an environment variable, see [Get started with the HarxitFlow API](/api-reference-api-examples).
...
1. To retrieve your current `user_id`, call the `/whoami` endpoint:
    ...
    ```
    {"id":"07e5b864-e367-4f52-b647-a48035ae7e5e","username":"harxitflow","profile_image":null,"store_api_key":null,"is_active":true,"is_superuser":true,"create_at":"2025-05-08T17:59:07.855965","updated_at":"2025-05-28T19:00:42.556460","last_login_at":"2025-05-28T19:00:42.554338","optins":{"github_starred":false,"dialog_dismissed":true,"discord_clicked":false,"mcp_dialog_dismissed":true}}
    ```
...
This endpoint uploads files to your HarxitFlow server's file management system.
To use an uploaded file in a flow, send the file path to a flow with a [**Read File** component](/read-file).

The default file limit is 1024 MB. To configure this value, change the `HARXITFLOW_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).
...
This component loads files into flows from your local machine or HarxitFlow file management.
