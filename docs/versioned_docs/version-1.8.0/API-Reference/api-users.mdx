...
Use the `/users` endpoint to manage user accounts in HarxitFlow.

## Add user

Create a new user account with a given username and password.

Requires authentication as a superuser if the HarxitFlow server has authentication enabled.
...
The user's UUID is stored in `user_id` in the HarxitFlow database, and returned as `id` in the `/users` API response.
This `user_id` key is specifically for HarxitFlow user management.
