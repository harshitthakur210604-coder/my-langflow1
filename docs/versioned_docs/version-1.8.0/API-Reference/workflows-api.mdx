...
The Workflow API provides programmatic access to execute HarxitFlow workflows synchronously or asynchronously.
...
The Workflow API is part of the HarxitFlow Developer v2 API and offers enhanced workflow execution capabilities compared to the v1 `/run` endpoint.
...
url = f"{HARXITFLOW_SERVER_URL}/api/v2/workflows"
headers = {
...
const url = `${HARXITFLOW_SERVER_URL}/api/v2/workflows`;
...
url = f"{HARXITFLOW_SERVER_URL}/api/v2/workflows/stop"
headers = {
    "Content-Type": "application/json",
    "x-api-key": HARXITFLOW_API_KEY
}
...
const url = `${HARXITFLOW_SERVER_URL}/api/v2/workflows/stop`;
...
To find the component ID in the HarxitFlow UI, open your flow in HarxitFlow, click the component, and then click **Controls**. The component ID is at the top of the **Controls** pane.
