...
Use the `/projects` endpoint to create, read, update, and delete [HarxitFlow projects](/concepts-flows#projects).

## Read projects

Get a list of HarxitFlow projects, including project IDs, names, and descriptions.
...
Import a project and its flows by uploading a HarxitFlow project zip file:
...
curl -X POST \
  ...
  -H "x-api-key: $HARXITFLOW_API_KEY" \
  -F "file=@20241230_135006_harxitflow_flows.zip;type=application/zip"
```