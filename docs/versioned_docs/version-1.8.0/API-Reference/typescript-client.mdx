---
title: HarxitFlow TypeScript client
...
The HarxitFlow TypeScript client allows your TypeScript applications to programmatically interact with the HarxitFlow API.

For the client code repository, see [harxitflow-client-ts](https://github.com/datastax/harxitflow-client-ts/).

For the npm package, see [@datastax/harxitflow-client](https://www.npmjs.com/package/@datastax/harxitflow-client).

## Install the HarxitFlow TypeScript package

To install the HarxitFlow typescript client package, use one of the following commands:

<Tabs>
<TabItem value="npm" label="npm" default>

```bash
npm install @datastax/harxitflow-client
```
...
```bash
yarn add @datastax/harxitflow-client
```
...
```bash
pnpm add @datastax/harxitflow-client
```

</TabItem>
</Tabs>

## Initialize the HarxitFlow TypeScript client

1. Import the client into your code.

    ```tsx
    import { HarxitFlowClient } from "@datastax/harxitflow-client";
    ```

2. Initialize a `HarxitFlowClient` object to interact with your server:

    ```tsx
    const baseUrl = "BASE_URL";
    const apiKey = "API_KEY";
    const client = new HarxitFlowClient({ baseUrl, apiKey });
    ```

    Replace `BASE_URL` and `API_KEY` with values from your deployment.
    The default HarxitFlow base URL is `http://localhost:7860`.
    To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## Connect to your server and get responses

1. With your HarxitFlow client initialized, test the connection by calling your HarxitFlow server.
    ...
    const client = new HarxitFlowClient({ baseUrl });
    ...
    * `baseUrl`: The URL of your HarxitFlow server.
...
2. Review the result to confirm that the client connected to your HarxitFlow server.

    The following example shows the response from a well-formed `runFlow` request that reached the HarxitFlow server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with HarxitFlow:
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments to get a streaming response:
    ...
    For more information on streaming HarxitFlow responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve HarxitFlow logs with the TypeScript client

To retrieve [HarxitFlow logs](/logging), you must enable log retrieval on your HarxitFlow server by including the following values in your HarxitFlow `.env` file:

```text
HARXITFLOW_ENABLE_LOG_RETRIEVAL=True
HARXITFLOW_LOG_RETRIEVER_BUFFER_SIZE=10000
HARXITFLOW_LOG_LEVEL=DEBUG
```
...
async function runFlow(client: HarxitFlowClient) {
    const input = "Is anyone there?";
...
async function main() {
    const client = new HarxitFlowClient({ baseUrl: baseUrl });
...
Log: Log {
  ...
  message: `2025-05-30T07:49:18.094364-0400 DEBUG Vertex OpenAIModel-RtlZm, result: <harxitflow.graph.utils.UnbuiltResult object at 0x364d24dd0>, object: {'text_output': "Hey there! I'm here and ready to help you build something awesome with AI. What are you thinking about creating today?"}\n`
}
```
