...
HarxitFlow includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses).
...
To be compatible with HarxitFlow's OpenAI Responses API endpoint, your flow and request must adhere to the following requirements:
...
HarxitFlow uses `x-api-key` headers for authentication, while OpenAI uses `Authorization: Bearer` headers.
When sending requests to HarxitFlow with OpenAI client libraries, you must configure custom headers and include an `api_key` configuration.
...
In the following examples, replace the values for `HARXITFLOW_SERVER_URL`, `HARXITFLOW_API_KEY`, and `FLOW_ID` with values from your deployment.
<Tabs groupId="client">
...
client = OpenAI(
  base_url="HARXITFLOW_SERVER_URL/api/v1/",
  default_headers={"x-api-key": "HARXITFLOW_API_KEY"},
...
|--------|----------|-------------|---------|
| `x-api-key` | Yes | Your HarxitFlow API key for authentication | `"sk-..."` |
| `Content-Type` | Yes | Specifies the JSON format | `"application/json"` |
| `X-HARXITFLOW-GLOBAL-VAR-*` | No | Global variables for the flow | `"X-HARXITFLOW-GLOBAL-VAR-API_KEY: sk-..."` For more, see [Pass global variables to your flows in headers](#global-var). |
...
The response contains fields that HarxitFlow sets dynamically and fields that use OpenAI-compatible defaults.
...
Fields set dynamically by HarxitFlow:
...
```bash
curl -X POST \
  "http://$HARXITFLOW_SERVER_URL/api/v1/responses" \
  -H "x-api-key: $HARXITFLOW_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "$FLOW_ID",
...
The `/responses` endpoint accepts global variables as custom HTTP headers with the format `X-HARXITFLOW-GLOBAL-VAR-{VARIABLE_NAME}`.
...
This example demonstrates passing an `OPENAI_API_KEY` variable, which is a variable HarxitFlow automatically detects from environment variables, with two custom variables for `USER_ID` and `ENVIRONMENT`. The variables don't have to be created in HarxitFlow's Global Variables section - you can pass any variable name in the `X-HARXITFLOW-GLOBAL-VAR-{VARIABLE_NAME}` header format.
...
Variables passed with `X-HARXITFLOW-GLOBAL-VAR-{VARIABLE_NAME}` are always available to your flow, regardless of whether they exist in the database.

If your flow components reference variables that aren't provided in headers or your HarxitFlow database, the flow fails by default.
