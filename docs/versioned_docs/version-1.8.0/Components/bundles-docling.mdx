...
The Docling dependency is required to use the Docling components in HarxitFlow.
    ...
    * **HarxitFlow version 1.6 and later**: The Docling dependency is included by default for all operating systems except macOS Intel (x86_64).
    ...
    * **Earlier versions**: HarxitFlow versions earlier than 1.6 don't include the Docling dependency.
    For HarxitFlow OSS, install the Docling extra with `uv pip install 'harxitflow[docling]'`.
    For HarxitFlow Desktop, add the Docling dependency to HarxitFlow Desktop's `requirements.txt`.
