...
HarxitFlow integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing and chunking documents.
...
The Docling dependency is required to use the Docling components in HarxitFlow.

    * **HarxitFlow Desktop**: Set `HARXITFLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for HarxitFlow Desktop](/environment-variables#set-environment-variables-for-harxitflow-desktop).

    * **HarxitFlow OSS**:
        * If you installed `harxitflow` (`uv pip install harxitflow`), Docling is included automatically through bundled extras.
        * If you installed `harxitflow-base` directly, install Docling with an extra, for example `uv pip install "harxitflow-base[docling]"`.
    ...
    * **Earlier versions**: HarxitFlow versions earlier than 1.6 don't include the Docling dependency by default.
...
:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + HarxitFlow: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
