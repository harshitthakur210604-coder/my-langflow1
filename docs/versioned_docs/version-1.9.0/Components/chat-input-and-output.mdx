...
**Chat Input and Output** components are designed to handle conversational interactions in HarxitFlow.
...
For information about these data types, see [Use HarxitFlow data types](/data-types).
...
When using the HarxitFlow API, the API response includes the **Chat Output** `Message` object along with other response data from the flow run.
...
* [HarxitFlow quickstart](/get-started-quickstart): Create and run a basic agent flow.
* **Basic Prompting** template: Create an LLM-based chat flow that accepts chat input as well as a prompt with additional instructions for the LLM. Many other HarxitFlow templates also use **Chat Input and Output** components.
...
### Send chat input with the HarxitFlow API

You can use the HarxitFlow API to run a flow by sending input to a **Chat Input** component:
...
When triggering flows with the HarxitFlow API, the payload must contain the values for the **Chat Input** component's input parameters, such as `input_value`.
...
For more information, see [Trigger flows with the HarxitFlow API](/concepts-publish).