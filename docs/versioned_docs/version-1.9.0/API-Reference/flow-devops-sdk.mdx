...
Instead of manually exporting, sharing, and importing flow JSON files from the HarxitFlow UI, the Flow DevOps toolkit offers terminal-based workflows for versioning, environment variables, testing, and deployment.

## Prerequisites

- [Install and start HarxitFlow](/get-started-installation)
- Create a [HarxitFlow API key](/api-keys-and-authentication)
- Install the HarxitFlow `lfx` package
   ...
   2. Activate the virtual environment.
      ...
      ```
   3. Install the HarxitFlow LFX package in the virtual environment:
      ```bash
      uv pip install lfx
      ```
   4. Run Flow DevOps Toolkit commands in the virtual environment that has LFX installed.

      Alternatively, you can run `uvx lfx` commands, or run LFX from the `src/lfx` directory in a cloned HarxitFlow repo.
      For more information, see the [HarxitFlow LFX README](https://github.com/harxitflow-ai/harxitflow/blob/main/src/lfx/README.md).
...
3. To initialize a project, run:
    ...
    ├── .github/
    │   └── workflows/
    │       ├── harxitflow-push.yml       # CI workflow
    │       ├── harxitflow-test.yml       # CI workflow
    │       └── harxitflow-validate.yml   # CI workflow
    ...
    ✔ Project scaffolded. Next steps:
      ...
      2. export HARXITFLOW_LOCAL_API_KEY=<key>   (Settings -> API Keys)
...
4. Add your HarxitFlow API key to your `.env` file, or export it within the terminal session.
    ...
    For example, to add a HarxitFlow API key to a local HarxitFlow server, set:

    ```text
    export HARXITFLOW_LOCAL_API_KEY=HARXITFLOW_API_KEY
    ```
...
7. In the HarxitFlow UI, open the Simple Agent flow and change the flow.
...
9. To pull the reported changes from the HarxitFlow server to your local project folder, run `lfx pull`.
10. To _push_ flow changes from flows stored locally in `demo-project/flows` to the HarxitFlow server, run `lfx push`.

## Validate flows

The Flow DevOps SDK can validate that local flows are correctly formed before pushing to the HarxitFlow with `lfx validate`.
...
2. Optionally, you can now share and serve the flow by keeping the flow JSON and `requirements.txt` in the same environment.
   To serve the flow without the HarxitFlow UI, do the following:
   ...
   4. To set a HarxitFlow API key, run:
   ```
   export HARXITFLOW_API_KEY=HARXITFLOW_API_KEY
   ```

   5. To serve the flow without the HarxitFlow UI, pass the flow JSON path to the `lfx serve` command:
   ```
...
production:
  ...
  api_key_env: HARXITFLOW_PRODUCTION_API_KEY
```

Each entry contains a `url` for the HarxitFlow base URL, and an `api_key_env` field.
...
`environments.yaml` is distinct from the HarxitFlow or LFX `.env` file.
`environments.yaml` controls which remote HarxitFlow instance you're deploying flows to, flow versioning, and environment variable _names_ for API keys.
The `.env` contains runtime values for the HarxitFlow server, and might also contain _actual secret values_, so the `.env` should not be committed to version control.

Commands that call a HarxitFlow server over HTTP, such as `lfx pull` or `lfx push`, use `--env ENVIRONMENT_NAME` to determine which HarxitFlow instance to send the request to.
...
This command will send the request to the HarxitFlow base URL at `http://127.0.0.1:7860` using a HarxitFlow API key named `HARXITFLOW_LOCAL_API_KEY`.
