...
for file in "${SH_FILES[@]}"; do
  ...
  if [[ "$MODE" == "execute" ]]; then
    ...
    if ! bash "$file" >/tmp/harxitflow-curl-example.out 2>/tmp/harxitflow-curl-example.err; then
      echo "FAIL  $rel (execution)"
      print_failure_logs "/tmp/harxitflow-curl-example.out" "/tmp/harxitflow-curl-example.err"
      ((FAIL+=1))
      continue
    fi
  fi
