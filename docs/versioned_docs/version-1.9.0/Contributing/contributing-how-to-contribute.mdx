---
title: Contribute to HarxitFlow
...
This guide is intended to help you start contributing to HarxitFlow.
As an open-source codebase in a rapidly developing field, HarxitFlow welcomes contributions, whether it be in the form of a new feature, improved infrastructure, or better documentation.

To contribute code or documentation to HarxitFlow, follow the [pull request guide](#open-a-pull-request).

## HarxitFlow services overview

This overview will help you understand how to set up your development environment.

HarxitFlow consists of two main services:
...
When you install HarxitFlow as an end user, the backend serves the frontend internally, making both services available on a single port (`7860` by default).

## Install HarxitFlow from source

Install HarxitFlow from source by forking the repository and setting up your development environment.
...
### Clone the HarxitFlow repository
...
```bash
git remote add FORK_NAME https://github.com/GIT_USERNAME/harxitflow.git
```
Replace the following:
* `FORK_NAME`: A name for your fork of the repository
* `GIT_USERNAME`: Your Git username

### Run HarxitFlow from source

You can run HarxitFlow from source after cloning the repository, even if you aren't contributing to the codebase.
This builds the frontend and serves it through the backend on port `7860`.

The instructions below are for running HarxitFlow from source. For development with hot reload, see [Set up development environment](#set-up-your-harxitflow-development-environment).

<details>
<summary>Run from source on macOS/Linux</summary>

In your terminal, navigate to the root of the HarxitFlow directory, and then run `make run_cli`.
...
The HarxitFlow frontend is served at `http://localhost:7860`.
</details>

<details>
<summary>Run from source with Windows CMD</summary>

To run HarxitFlow from source on Windows, you can use the HarxitFlow repository's included scripts, or run the commands in the terminal.

Do one of the following:

* To install and run HarxitFlow with the included Windows Batch file, navigate to the `scripts/windows` directory, and then run the `build_and_run.bat` file.

* To run HarxitFlow from the Windows Command Line:
    ...
    2. Copy the contents of the built `src/frontend/build` directory to `src/backend/base/harxitflow/frontend`.
...
* To install and run HarxitFlow with the included scripts, navigate to the `scripts/windows` directory, and then run the `build_and_run.ps1` file.

* To run HarxitFlow from a PowerShell terminal:
...
### Set up your HarxitFlow development environment
...
If you just want to run HarxitFlow locally without making code changes, see [Run HarxitFlow from source](#run-harxitflow-from-source).

<Tabs>
<TabItem value="linux-macos" label="Linux or macOS" default>

1. Set up the HarxitFlow development environment:
...
Since Windows doesn't include `make`, building and running HarxitFlow from source uses `npm` and `uv`.

To set up the HarxitFlow development environment, run the frontend and backend in separate terminals:
...
2. Fork the [HarxitFlow GitHub repository](https://github.com/harxitflow-ai/harxitflow).
...
4. From the root of your local HarxitFlow fork, change to the `/docs` directory:
...
6. To edit and create content, work with the `.mdx` files in the `harxitflow/docs/docs` directory.

    Create new files in `.mdx` format.

    Navigation is defined in `harxitflow/docs/sidebars.js`.
...
* Pull request titles appear in HarxitFlow's release notes, so they should explain what the PR does as explicitly as possible.
* Pull requests should strive to fix one thing **only**, and should contain a good description of what is being fixed.

3. A HarxitFlow maintainer will review your pull request and may request changes, so ensure you pay attention to your PRs. Thanks for your contribution!
