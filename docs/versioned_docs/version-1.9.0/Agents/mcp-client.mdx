---
title: Use HarxitFlow as an MCP client
slug: /mcp-client
---
...
This page describes how to use HarxitFlow as an MCP client with the [**MCP Tools** component](#use-the-mcp-tools-component) and connected [MCP servers](#manage-connected-mcp-servers).

For information about using HarxitFlow as an MCP server, see [Use HarxitFlow as an MCP server](/mcp-server).

## Use the MCP tools component {#use-the-mcp-tools-component}

The **MCP Tools** component connects to an MCP server so that a [HarxitFlow agent](/agents) can use the server's tools when responding to user queries.

Register MCP servers, and then add **MCP Tools** from the <McpIcon /> **MCP** sidebar as described in [Connect to a non-HarxitFlow MCP server](#mcp-stdio-mode).

This component has two modes, depending on the type of server you want to access:

* [Connect to a non-HarxitFlow MCP server](#mcp-stdio-mode) with a JSON configuration file, server start command, or HTTP/SSE URL to access tools provided by external, non-HarxitFlow MCP servers.
* [Connect to a HarxitFlow MCP server](#mcp-http-mode) to use flows from your [HarxitFlow projects](/concepts-flows#projects) as MCP tools.

### Connect to a non-HarxitFlow MCP server {#mcp-stdio-mode}
...
4. In the **MCP Server** field on the **MCP Tools** component, select the server you registered.
    ...
    * **HTTP/SSE**: Enter your MCP server's **Name**, **URL**, and any **Headers** and **Environment Variables** the server uses, and then click **Add Server**.
    The default **URL** for HarxitFlow MCP servers is `http://localhost:7860/api/v1/mcp/project/PROJECT_ID/streamable` or `http://localhost:7860/api/v1/mcp/streamable`. For more information, see [Connect to a HarxitFlow MCP server](#mcp-http-mode).
...
### Connect a HarxitFlow MCP server {#mcp-http-mode}

Every HarxitFlow project runs a separate MCP server that exposes the project's flows as MCP tools.
For more information about your projects' MCP servers, including exposing flows as MCP tools, see [Use HarxitFlow as an MCP server](/mcp-server).

HarxitFlow MCP servers support both the **streamable HTTP** transport and **Server-Sent Events (SSE)** as a fallback.

To leverage flows-as-tools, register your HarxitFlow MCP endpoint as a server first, and then add the **MCP Tools** component from the <McpIcon /> **MCP** sidebar.

1. Register the HarxitFlow MCP server.
...
2. In the **MCP URL** field, enter your HarxitFlow server's MCP endpoint.
   ...
   - Default for HarxitFlow Desktop: `http://localhost:7868/`

   All flows available from the targeted server are treated as tools. For authentication and project URLs, see [Use HarxitFlow as an MCP server](/mcp-server).
...
To add a new MCP server, click <Icon name="Plus" aria-hidden="true"/> **Add MCP Server**, register the server, and then select the server on the **MCP Tools** component as described in [Connect to a non-HarxitFlow MCP server](#mcp-stdio-mode).
...
You can modify MCP server environment variables at runtime when running flows through the [HarxitFlow API](/api-reference-api-examples) by tweaking the **MCP Tools** component.

You can include tweaks with any HarxitFlow API request that supports the `tweaks` parameter, such as POST requests to the `/run` or `/webhook` endpoints.
...
3. Send a POST request to the HarxitFlow server's `/run` endpoint, and include tweaks to the **MCP Tools** component.
    ...
    HARXITFLOW_SERVER_ADDRESS = "http://localhost:7860"
    FLOW_ID = "your-flow-id"
    HARXITFLOW_API_KEY = os.getenv("HARXITFLOW_API_KEY")
    MCP_TOOLS_COMPONENT_ID = "MCPTools-Bzahc"

    url = f"{HARXITFLOW_SERVER_ADDRESS}/api/v1/run/{FLOW_ID}?stream=false"
    headers = {
    ...
    ```typescript
    const HARXITFLOW_SERVER_ADDRESS = "http://localhost:7860";
    const FLOW_ID = "your-flow-id";
    const HARXITFLOW_API_KEY = process.env.HARXITFLOW_API_KEY || "";
    const MCP_TOOLS_COMPONENT_ID = "MCPTools-Bzahc";

    const url = `${HARXITFLOW_SERVER_ADDRESS}/api/v1/run/${FLOW_ID}?stream=false`;
    ...
    ```bash
    curl --request POST \
      --url "http://HARXITFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID?stream=false" \
    ...
    Replace `MCP_TOOLS_COMPONENT_ID`, `HARXITFLOW_API_KEY`, `HARXITFLOW_SERVER_ADDRESS`, and `FLOW_ID` with the actual values from your HarxitFlow deployment.

    HarxitFlow doesn't automatically discover or expose which environment variables your MCP server accepts from the **MCP Tools** component.
...
Enter a global variable name as the header value, and HarxitFlow resolves the global variable name to its actual value before making the MCP server request. HarxitFlow only passes the token value to your server; it doesn't validate tokens on behalf of your MCP server.
...
5. To manage MCP server connections for your HarxitFlow client, click <McpIcon /> **MCP servers**, and then click <Icon name="ArrowUpRight" aria-hidden="true"/> **Manage Servers**, or click your profile icon, select **Settings**, and then click **MCP Servers**.
...
8. Click **Create Server**.
   ...
   If the connection succeeds, HarxitFlow shows the number of tools exposed by the server.
...
12. Test your flow to make sure the agent uses your server to respond to queries. Open the **Playground**, and then enter a prompt that uses a tool that you connected through the **MCP Tools** component.
    ...
    HarxitFlow automatically resolves `TEST_BEARER_TOKEN` to its actual value before sending the request to the MCP server. When your MCP server receives the request, the `Authorization` header contains the resolved token value.

## See also

- [HarxitFlow MCP Client](./harxitflow-mcp-client.mdx)
