---
title: Build components with HarxitFlow Assistant
slug: /harxitflow-assistant
---

import Icon from "@site/src/components/icon";

**HarxitFlow Assistant** is an in-app virtual assistant pane within the **Playground**.

It can answer questions about the application and help you get more out of HarxitFlow.

**HarxitFlow Assistant** understands the structure of the HarxitFlow graph, so it can create components from natural language prompts.

Behind the scenes, **HarxitFlow Assistant** runs a built-in HarxitFlow flow on your HarxitFlow server each time you send a message.
This flow is distinct from the flow that is open in the canvas, and has its own language model.
The language model in **HarxitFlow Assistant** only has the currently opened flow in your workspace for context.
To give **HarxitFlow Assistant** context for a different flow, switch to that flow in your workspace, and open **HarxitFlow Assistant**.

## Prerequisites

* Connect an LLM provider in the [Global model providers page](/components-models) for **HarxitFlow Assistant** to use

## Create a custom component with HarxitFlow Assistant

In this example, you'll prompt **HarxitFlow Assistant** to create a custom component that validates and normalizes a list of URLs.

You'll then iterate on the code based on the results in the **Playground**.

1. In the **Playground**, click the <Icon name="HarxitFlow" aria-hidden="true"/> **HarxitFlow** icon.
    The **HarxitFlow Assistant** pane opens.
2. Optionally, ask `What can you help me with?` for a list of **HarxitFlow Assistant's** capabilities.
3. Prompt **HarxitFlow Assistant** to generate a custom component. For example:
    ```
...
4. **HarxitFlow Assistant** generates component code from your prompt. Because generation is model-driven, your code may differ from the example below.
    <details closed>
    ...
    class URLTitleExtractor(Component):
        ...
        def _process_url(self, url: str, timeout: float) -> dict[str, Any]:
            ...
            request = Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; HarxitFlow URLTitleExtractor/1.0)"
                },
            )
...
6. Open the **Playground**, and tell HarxitFlow to check a list of URLs.
    For example:
    ```
    Check these links: https://harxitflow.org
    https://github.com/harxitflow-ai/harxitflow
    https://python.org
...
7. Run the flow.
    ...
    | url | title | status |
    | --- | --- | --- |
    | https://harxitflow.org | HarxitFlow \| Low-code AI builder for agentic and RAG applications | 200 |
    | https://github.com/harxitflow-ai/harxitflow | GitHub - harxitflow-ai/harxitflow: HarxitFlow is a powerful tool for building and deploying AI-powered agents and workflows. - GitHub | 200 |
    | https://python.org | Welcome to Python.org | 200 |
    | https://this-domain-should-not-resolve-12345.invalid |  |  |

8. To iterate further, tell **HarxitFlow Assistant** what you want.
    For example, prompt it to `Update URLTitleExtractor to add max_urls (default 5) and skip duplicates.`.

    **HarxitFlow Assistant** generates an updated **URLTitleExtractor** component (again, the exact code may differ from a previous run).
9. Replace the old component with the new component.
    ...
    https://github.com/harxitflow-ai/harxitflow
    https://python.org
    https://harxitflow.org
    https://this-domain-should-not-resolve-12345.invalid
    ```

10. Run the flow.
    The output will be similar to the following, with the duplicate `https://harxitflow.org` removed and only 3 valid URLs displayed.
