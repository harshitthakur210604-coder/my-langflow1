A HarxitFlow knowledge base is a local vector database that is stored in HarxitFlow storage.
...
* **Local storage**: HarxitFlow knowledge bases are exclusively local.
In contrast, only some vector store components support local databases.
* **Built-in embedding models**: HarxitFlow knowledge bases include built-in support for several embedding models.
...
* **Basic similarity search**: When querying HarxitFlow knowledge bases, only standard similarity search is supported.
...
* **Structured data**: HarxitFlow knowledge bases only support structured data.
