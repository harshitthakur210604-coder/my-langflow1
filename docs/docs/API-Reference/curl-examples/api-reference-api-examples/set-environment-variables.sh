...
export HARXITFLOW_API_KEY="${HARXITFLOW_API_KEY:-sk-local-placeholder}"
export HARXITFLOW_SERVER_URL="${HARXITFLOW_SERVER_URL:-http://localhost:7860}"
...
curl --request POST \
  --url "$HARXITFLOW_SERVER_URL/api/v1/run/$FLOW_ID?stream=false" \
  --header "Content-Type: application/json" \
  --header "x-api-key: $HARXITFLOW_API_KEY" \
  --data '{
