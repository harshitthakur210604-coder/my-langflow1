---
title: Get started with the HarxitFlow API
slug: /api-reference-api-examples
---
import CodeSnippet from '@site/src/components/CodeSnippet';
import exampleApiReferenceApiExamplesFormHarxitFlowApiRequests from '!!raw-loader!@site/docs/API-Reference/curl-examples/api-reference-api-examples/form-harxitflow-api-requests.sh';
...
import examplePythonApiReferenceApiExamplesFormHarxitFlowApiRequests from '!!raw-loader!@site/docs/API-Reference/python-examples/api-reference-api-examples/form-harxitflow-api-requests.py';
import exampleJavascriptApiReferenceApiExamplesFormHarxitFlowApiRequests from '!!raw-loader!@site/docs/API-Reference/javascript-examples/api-reference-api-examples/form-harxitflow-api-requests.js';
...
You can use the HarxitFlow API for programmatic interactions with HarxitFlow, such as the following:

* Create and edit flows, including file management for flows.
* Develop applications that use your flows.
* Develop custom components.
* Build HarxitFlow as a dependency of a larger application, codebase, or service.
* Contribute to the overall HarxitFlow codebase.

To view and test all available endpoints, you can access the HarxitFlow API's OpenAPI specification at your HarxitFlow deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the HarxitFlow API in a script, see the [HarxitFlow quickstart](/get-started-quickstart).
...
## Form HarxitFlow API requests

While individual options vary by endpoint, all HarxitFlow API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a HarxitFlow API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

<Tabs>
<TabItem value="Python" label="Python" default>

<CodeSnippet source={examplePythonApiReferenceApiExamplesFormHarxitFlowApiRequests} language="python" />

</TabItem>
<TabItem value="JavaScript" label="JavaScript">

<CodeSnippet source={exampleJavascriptApiReferenceApiExamplesFormHarxitFlowApiRequests} language="javascript" />

</TabItem>
<TabItem value="curl" label="curl">

<CodeSnippet source={exampleApiReferenceApiExamplesFormHarxitFlowApiRequests} language="bash" />
...
By default, local deployments serve the HarxitFlow API at `http://localhost:7860/api`.

Remotely hosted HarxitFlow deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:HARXITFLOW_PORT/api`.

You can configure the HarxitFlow port number in the `HARXITFLOW_PORT` [environment variable](/environment-variables).

* `https://UUID.ngrok.app/api`
* `http://IP_OR_DNS/api`
* `http://IP_OR_DNS:HARXITFLOW_PORT/api`

### Authentication

In HarxitFlow versions 1.5 and later, most API endpoints require authentication with a HarxitFlow API key in either an `x-api-key` header or query parameter.
...
HarxitFlow API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The HarxitFlow API serves `/v1` and `/v2` endpoints.
...
Commonly used values in HarxitFlow API requests include your [HarxitFlow server URL](#base-url), [HarxitFlow API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some HarxitFlow API requests

Once you have your HarxitFlow server URL, try calling these endpoints that return HarxitFlow metadata.

### Health check

Returns the health status of the HarxitFlow database and chat services:
...
HarxitFlow provides an additional `GET /health` endpoint.
This endpoint is served by uvicorn before HarxitFlow is fully initialized, so it's not reliable for checking HarxitFlow service health.

### Get version

Returns the current HarxitFlow API version:
...
Returns configuration details for your HarxitFlow deployment.
Requires a [HarxitFlow API key](/api-keys-and-authentication).
...
Returns a dictionary of all HarxitFlow components.
Requires a [HarxitFlow API key](/api-keys-and-authentication).
...
Because you can run HarxitFlow as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the HarxitFlow database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the HarxitFlow codebase, you won't directly call most of the HarxitFlow endpoints.
...
The following endpoints are useful for developing applications with HarxitFlow and administering HarxitFlow deployments with one or more users.
...
* Deployment details:
  * GET `/v1/version`: Return HarxitFlow version. See [Get version](/api-reference-api-examples#get-version).
...
* Custom components: You might use these endpoints when developing custom HarxitFlow components for your own use or to share with the HarxitFlow community:
  * GET `/v1/all`: Return all available HarxitFlow component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are most often used when contributing to the HarxitFlow codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
* MCP servers: The following endpoints are for managing HarxitFlow MCP servers and MCP server connections.
They aren't typically called directly; instead, they are used to drive internal functionality in the HarxitFlow frontend and when running flows that call MCP servers.
HarxitFlow MCP servers support both streamable HTTP and SSE transport.
...
* Use the HarxitFlow API to [run a flow](/api-flows-run).
* Use the HarxitFlow API to [upload files](/api-files).
* Use the HarxitFlow API to [get flow logs](/api-logs).
* Explore all endpoints in the [HarxitFlow API specification](/api).
