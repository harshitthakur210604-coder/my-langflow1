...
This tutorial shows you how to connect a JavaScript application to a [HarxitFlow agent](/agents).
...
In this example, the application sends a customer's email address to the HarxitFlow agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    const HARXITFLOW_SERVER_ADDRESS = 'HARXITFLOW_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';
    const HARXITFLOW_API_KEY = 'HARXITFLOW_API_KEY';
    ...
    async function runAgentFlow(): Promise<void> {
        try {
            ...
            const client = new HarxitFlowClient({
                baseUrl: HARXITFLOW_SERVER_ADDRESS,
                apiKey: HARXITFLOW_API_KEY
            });

            console.log(`Connecting to HarxitFlow server at: ${HARXITFLOW_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your HarxitFlow server is running and accessible at:', HARXITFLOW_SERVER_ADDRESS);
                }
    ...
    // Run the function
    console.log('Starting HarxitFlow Agent...\n');
...
3.  Save and run the script to send the request and test the flow.
    ...
    ```
    Starting HarxitFlow Agent...

    Connecting to HarxitFlow server at: http://localhost:7860
