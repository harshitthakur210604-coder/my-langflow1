...
:::info
Voice mode is not available in HarxitFlow Desktop.
To use voice mode, [Install the HarxitFlow OSS Python package](/get-started-installation#install-and-run-the-harxitflow-oss-python-package).
:::

You can use HarxitFlow's voice mode to interact with your flows verbally through a microphone and speakers.
...
* An [OpenAI](https://platform.openai.com/) account and an OpenAI API key because HarxitFlow uses the OpenAI API to process voice input and generate responses.
...
5. Enter your OpenAI API key, and then click **Save**. HarxitFlow saves the key as a [global variable](/configuration-global-variables).
...
HarxitFlow saves this key as a global variable.
...
HarxitFlow exposes two OpenAI Realtime API-compatible websocket endpoints for your flows.
...
The HarxitFlow API's websockets endpoints require an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and they support an optional [ElevenLabs](https://elevenlabs.io) integration with an ElevenLabs API key.
...
This is the mode used in the HarxitFlow **Playground**.
...
If omitted, HarxitFlow uses the flow ID as the [session ID](/session-id).
