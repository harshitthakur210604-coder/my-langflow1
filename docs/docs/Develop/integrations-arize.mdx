...
To enable Arize tracing, set the required Arize environment variables in your HarxitFlow deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating HarxitFlow and Arize are also available in the Arize documentation:

* [HarxitFlow tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/harxitflow/harxitflow-tracing)
* [HarxitFlow tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/harxitflow/harxitflow-tracing)
:::
...
2. In the root of your HarxitFlow application, edit your existing HarxitFlow `.env` file or create a new one.
...
4. Start your HarxitFlow application with your `.env` file:
...
1. In HarxitFlow, run a flow that has an LLM-driven component, such as an **Agent** component or any language model component.
...
3. To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each HarxitFlow execution generates two traces in Arize:

    * The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    * The `UUID` trace is the trace of the HarxitFlow components.
